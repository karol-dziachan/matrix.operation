...
public:
    ...
    bool b_add(CMatrix& pc_other);
    void operator+(CMatrix& pc_other);
    bool b_subtraction( CMatrix& c_other);
    void operator-( CMatrix& pc_other);
    bool b_multiply( CMatrix& pc_other);
    void operator*=( CMatrix& pc_other);
    bool b_multiply_by_number(T t_number);
    ...
    T t_scalar_calculate(CMatrix& pc_other);
...
private:
    ...
    T  t_convert_int(string s_value);
    int  i_convert_numeral(char c_value);
    T  t_convert_double_float( string s_value);
    T t_pow(int i_pow, double i_base);
    CError* c_error;
...
CMatrix<T> ::CMatrix(string s_path) {
 ...
 if (reader.is_open()) {
     char row[MAX_SIZE];
     while (reader.getline(row, MAX_SIZE)) {
         v_tmp = s_split(row);
         i_how_many_rows++;
     ...
     if (i_how_many_cols * i_how_many_rows > v_row.size())
     {
         ...
         c_error->v_add_error(NOT_ENOUGH_DATA);
     ...
     {
         int i_counter = v_row.size();
         c_error->v_add_error(TO_MANY_DATA);
    ...
    {
        c_error->v_add_error(WRONG_PATH);
        v_allocate_space(DEFAULT_DIM, DEFAULT_DIM);
    }
...
T CMatrix<T> ::t_parse(string c_open)
{
    if(b_contains_not_number(c_open)) {
        c_error->v_add_error(WRONG_DATA);
        return DEFAULT_VALUE;
    }

    if(b_contains(c_open, '.') || b_contains(c_open, ','))
        return t_convert_double_float(c_open);
    else
        return t_convert_int(c_open);
}
...
template <typename T>
T CMatrix<T> :: t_convert_int(string s_value)
{
    T t_tmp = 0;
    int i_size = s_value.size();

    for(int ii = 0 ; ii < i_size; ii++) {
        t_tmp += (i_convert_numeral(s_value[ii]) * t_pow(s_value.size() - 1 -ii, 10));
    }
...
template<typename T>
T CMatrix<T> ::t_convert_double_float(string s_value)
{
    
    T t_tmp = 0;
    int i_size = s_value.size();
    bool b_after_separ = false;
    int i_after_separ = 0;

    int b_before_separ = 0;

    while (s_value[b_before_separ] != '.')
        b_before_separ += 1; 

    for (int ii = 0; ii < i_size; ii++) {
        
        if (s_value[ii] == '.' || s_value[ii] == ',')
        {
            b_after_separ = true;
            i_after_separ += 1;
        }
        else if(!b_after_separ && (s_value[ii] != '.' && s_value[ii] != ',') )
            t_tmp += (i_convert_numeral(s_value[ii]) * t_pow(b_before_separ - 1 - ii, 10));
        else if (b_after_separ && (s_value[ii] != '.' && s_value[ii] != ',') )
        {
            t_tmp += (i_convert_numeral(s_value[ii]) * t_pow(i_after_separ, 0.1f));
            i_after_separ += 1;
        }
...
template <typename T>
T CMatrix<T> :: t_pow(int i_pow, double i_base)
{
...
template <typename T>
bool CMatrix<T> ::b_add(CMatrix& pc_other)
{
    if (i_cols != pc_other.i_cols || i_rows != pc_other.i_rows)
    {
        c_error->v_add_error(ADD_NOT_POSSIBLE);
        return false;
    }
...
template <typename T>
bool CMatrix<T> ::b_subtraction(CMatrix<T>& pc_other)
{
    if (i_cols != pc_other.i_cols || i_rows != pc_other.i_rows)
    {
        c_error->v_add_error(SUB_NOT_POSSIBLE);
        return false;
    }
...
template<typename T>
void CMatrix<T> ::operator-(CMatrix<T>& pc_other) {
  if(!b_subtraction(pc_other))
      throw CException(c_error->s_last_error());
}


template <typename T>
bool CMatrix<T> ::b_multiply(CMatrix<T>& pc_other)
{
    if (i_rows != pc_other.i_cols)
    {
        c_error->v_add_error(MULT_NOT_POSSIBLE);
        return false;
    }
...
void CMatrix<T> ::operator=(CMatrix<T>& pc_other) {
    if (this == &pc_other)
    {
        c_error->v_add_error(USELESS_ASSIGNMENT);
        return;
    }
...
template <typename T>
bool CMatrix<T> ::b_multiply_by_number(T t_number)
{
    for (int ii = 0; ii < i_rows; ii++)
        for (int ij = 0; ij < i_cols; ij++)
...
void CMatrix<T> ::operator*=(T t_number) {
    if(!b_multiply_by_number(t_number))
        throw CException(c_error->s_last_error());
}
...
vector<T> CMatrix<T> ::v_create_vector_row(int i_pos)
{
    ...
    {
        vector<T> v_null;
        c_error->v_add_error(WRONG_POS_VEC);
        return v_null;
    }
...
template <typename T>
T CMatrix<T> ::t_scalar_calculate(CMatrix& pc_other)
{
    if(i_rows > 1 || pc_other.i_rows > 1 || i_cols != pc_other.i_cols)
    {
        c_error->v_add_error(WRONG_VEC_SIZES);
        return DEFAULT_VALUE; 
    }
    ...
    for(int ii =0 ; ii<i_cols; ii++)
        t_tmp += (pt_matrix[ii][0] * pc_other.pt_matrix[ii][0]);
