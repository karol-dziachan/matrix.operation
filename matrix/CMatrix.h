...
private:
    ...
    T i_pow(int i_pow, double i_base);
    CError* c_error;
...
T CMatrix<T> ::b_convert_double_float(string s_value)
{
    ...
    for (int ii = 0; ii < i_size; ii++) {
        ...
        if (s_value[ii] == '.' || s_value[ii] == ',')
        {
            ...
            t_tmp += (i_convert_numeral(s_value[ii]) * i_pow(b_before_separ - 1 - ii, 10));
        else if (b_after_separ && (s_value[ii] != '.' && s_value[ii] != ',') )
        {
            t_tmp += (i_convert_numeral(s_value[ii]) * i_pow(i_after_separ, 0.1f));
            i_after_separ += 1;
        }
...
template <typename T>
T CMatrix<T> :: i_pow(int i_pow, double i_base)
{
