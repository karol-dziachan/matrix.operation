...
public:
    ...
    void v_add(CMatrix& pc_other);
    void operator+( CMatrix& pc_other);
    void v_subtraction( CMatrix& c_other);
    void operator-( CMatrix& pc_other);
    void v_multiply( CMatrix& pc_other);
    void operator*=( CMatrix& pc_other);
    void v_multiply_by_number(T t_number);
...
template <typename T>
void CMatrix<T> ::v_add(CMatrix& pc_other)
{
...
template <typename T>
void CMatrix<T> ::v_subtraction(CMatrix<T>& pc_other)
{
    if (i_cols != pc_other.i_cols || i_rows != pc_other.i_rows)
    {
...
template<typename T>
void CMatrix<T> ::operator-(CMatrix<T>& pc_other) {
    v_subtraction(pc_other);
}


template <typename T>
void CMatrix<T> ::v_multiply(CMatrix<T>& pc_other)
{
...
template <typename T>
void CMatrix<T> ::v_multiply_by_number(T t_number)
{
    for (int ii = 0; ii < i_rows; ii++)
        for (int ij = 0; ij < i_cols; ij++)
